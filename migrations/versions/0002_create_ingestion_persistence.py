"""create shared ingestion and candidate persistence

Revision ID: 0002_ingestion_persistence
...
revision = "0002_ingestion_persistence"
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.UniqueConstraint(
            "idempotency_key", name="uq_source_records_idempotency_key"
        ),
    )
    op.create_index(
        "ix_source_external_identity",
        "source_records",
    ...
    op.create_table(
        ...
        sa.Column("metadata", _JSON, nullable=False),
        sa.CheckConstraint("ordinal >= 0", name="ck_evidence_fragments_ordinal"),
        sa.ForeignKeyConstraint(
            ["source_id"], ["source_records.source_id"], ondelete="CASCADE"
        ),
        sa.UniqueConstraint(
            "source_id", "ordinal", name="uq_evidence_source_ordinal"
        ),
        sa.UniqueConstraint(
            "source_id", "fragment_id", name="uq_evidence_source_fragment"
        ),
    )
    op.create_index(
    ...
    op.create_table(
        ...
        sa.UniqueConstraint(
            "idempotency_key", name="uq_candidate_idempotency_key"
        ),
    )
    op.create_index(
    ...
    op.create_table(
        ...
        sa.Column("id", _ROW_ID, primary_key=True, autoincrement=True),
        ...
        sa.UniqueConstraint(
            "change_id", "ordinal", name="uq_candidate_evidence_ordinal"
        ),
    )
    op.create_index(
        "ix_candidate_evidence_source",
        "candidate_evidence_refs",
        ["source_id"],
    )
    op.create_index(
        "ix_candidate_evidence_fragment",
        "candidate_evidence_refs",
        ["content_fragment_id"],
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_candidate_evidence_fragment", table_name="candidate_evidence_refs"
    )
    op.drop_index(
        "ix_candidate_evidence_source", table_name="candidate_evidence_refs"
    )
    op.drop_table("candidate_evidence_refs")

    op.drop_index(
        "ix_candidate_ingestion_run", table_name="candidate_graph_changes"
    )
    op.drop_index(
        "ix_candidate_target_status", table_name="candidate_graph_changes"
    )
    op.drop_index(
        "ix_candidate_review_status_created", table_name="candidate_graph_changes"
    )
    ...
    op.drop_index(
        "ix_evidence_fragments_source_ordinal", table_name="evidence_fragments"
    )
    op.drop_table("evidence_fragments")

    op.drop_index("ix_source_external_identity", table_name="source_records")
