# ADR-0005：ME-System 采用单一 ME-Core
...
仓库中现有 `services/me-graph-core/` 与 `me_graph_core` 是早期名称，不代表额外核心；它们将在当前输入持久化切片开始前迁移为 `me-core` / `me_core`，避免继续产生第三组件的错觉。

双图谱是 ME-Core 中的两个权威命名空间，不是两个独立后端。Source、Evidence、Candidate、Query、MCP、CLI 和后续 Adapter 都围绕同一个 ME-Core 运行，不再演化为平级核心或独立真相源。
...
```text
ME-System   产品、仓库与整体项目
ME-Core     唯一运行内核
ME-Brain    ME-Core 内的项目图谱域
ME-Who      ME-Core 内的用户理解图谱域
MCP / CLI   ME-Core 的薄访问前端
```
...
- 图谱与证据语义只在 `me_core` 中定义一次；
- MCP 和 CLI 只是同一应用服务的两个前端。
...
1. `ingestion/` 是 ME-Core 子系统，不是新产品；
2. `adapters/` 只负责来源到标准输入/候选的转换；
3. `mcp/` 和 `integrations/` 只负责协议适配，不定义图谱业务规则；
4. Domain Pack 只能扩展本体、Pass 和查询，不创建第二套 Store；
5. 所有权威写入最终经过同一 PostgreSQL 事务边界。

## 当前实施影响

Source Ledger 与 Candidate 持久化作为 ME-Core 内部摄取能力实现：
...
在此之前，优先保持一个 ME-Core、一个权威 PostgreSQL、一个图谱语义来源。