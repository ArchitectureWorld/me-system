...
两条产品线共享原始来源、时间、实体标识、权限、来源追踪和上下文协议，但不共享领域事实：
...
Monorepo 只是一种开发组织方式，不代表两个产品必须捆绑部署。ME-Who、ME-Brain 和 ME-Context 从第一天起都应拥有独立 API、数据库 Schema、测试边界和 Docker 镜像。
...
1. **来源优先**：高价值事实必须能够追溯到原始资料。
2. **权威数据与派生索引分离**：向量、图谱和摘要不能静默覆盖权威事实。
3. **用户拥有身份解释权**：ME-Who 的推断必须可查看、确认、修订、限制和删除。
4. **项目事实不受偏好污染**：ME-Who 只影响上下文策略，不改变 ME-Brain 的客观状态。
5. **渐进式上下文装载**：Agent 默认读取结构化摘要，必要时再下钻到证据和原文。
6. **领域原生而非万能本体**：先建立软件、科研和设计领域包，再逐步扩展。
7. **本地优先与可替换组件**：核心领域模型自建，解析、检索、图谱和连接器保持可替换。

## 当前阶段

仓库目前处于产品定义与基础架构阶段。下一阶段应优先完成：

1. Shared Source Ledger 与 Provenance 规范；
2. ME-Brain Schema v0.1；
3. ME-Who Schema v0.1；
4. Project Context Pack 与 Personal Context Pack；
5. 统一 Context Compiler 的输入输出协议；
6. 使用真实项目建立基线评估。
